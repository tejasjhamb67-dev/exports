...
export default function IndiaMap({ onSelect, selected, highlight, hoverRender }: {
  onSelect: (s: string | null) => void;
  selected: string | null;
  // Optional product-driven mode (ENGINEERING_PLAN §3): paint states by a
  // supplied intensity map (geometry key -> 0..1) and render product-specific
  // hover content. When omitted, the map behaves exactly as the BS1000 view.
  highlight?: Record<string, number>;
  hoverRender?: (state: string) => React.ReactNode;
}) {
  ...
  function fill(key: string) {
    if (hlMode) {
      const t = highlight![key];
      if (t == null) return "#eef1f5";
      return lerpColor("#f3ecdb", "#eab24a", Math.pow(t, 0.7));
    }
  ...
  function move(e: React.MouseEvent, key: string) {
    if (hlMode) {
      if (highlight![key] == null) { setHover(null); return; }
      setHover({ key, x: e.clientX, y: e.clientY }); return;
    }
  ...
  return (
    <div className="mapwrap" ref={wrapRef}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, flexWrap: "wrap", gap: 10 }}>
        {hlMode ? (
          <span className="zoom-hint">Origin states for this product · darker → higher share</span>
        ) : (
          <div className="seg">
            {METRICS.map((m) => (
        ...
        {selected ? <button className="btn" onClick={() => onSelect(null)}>← Reset · full India</button> : !hlMode ? <span className="zoom-hint">Hover a state · click to zoom in</span> : null}
      </div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ maxHeight: 620, display: "block" }}
        ...
        <g style={{ transform, transformOrigin: "0 0", transition: "transform 0.6s cubic-bezier(0.3,0.8,0.3,1)" }}>
          {Object.entries(states).map(([key, st]) => {
            ...
            const dim = (selected && !isSel) || (hlMode && highlight![key] == null && !isSel);
            return (
